...
async def get_organization(organization_request: OrganizationName):
    ...
    get_details = organization_manager_obj.get_organization_by_name(
...
async def create_new_organization(organization_request: CreateOrganization):
    ...
    print(
        "parametry w params req: ",
        organization_request.name,
        organization_request.display_name,
    )

    create_new_organization = organization_manager_obj.create_organization(
...
async def delete_organization(organization_request: OrganizationIdentifier):
    ...
    delete = organization_manager_obj.delete_organization_by_identifier(
...
async def modify_organization_router(organization_request: ModifyOrganization):
    modify = organization_manager_obj.modify_organization(
        ...
        name=(
            organization_request.name if organization_request.name is not None else None
        ),
        display_name=(
            organization_request.display_name
            if organization_request.display_name is not None
            else None
        ),
        logo_url=(
            organization_request.logo_url
            if organization_request.logo_url is not None
            else None
        ),
        primary_color=(
            organization_request.primary_color
            if organization_request.primary_color is not None
            else None
        ),
        background_color=(
            organization_request.background_color
            if organization_request.background_color is not None
            else None
        ),
    )
